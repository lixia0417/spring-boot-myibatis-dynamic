...
public abstract class AbstractDynamicDataSource<T extends DataSource> extends AbstractRoutingDataSource
	...
	protected Object determineCurrentLookupKey() {
		...
		Map<String, Object> configMap = RoutingDataSourceContext.getDBType();
		...
		return configMap.get(RoutingDataSourceContext.DATASOURCE_KEY);
	}
	...
	private void verifyAndInitDataSource() {
		Map<String, Object> configMap = RoutingDataSourceContext.getDBType();
		Object obj = this.targetDataSources.get(configMap.get(RoutingDataSourceContext.DATASOURCE_KEY));
		if (obj != null) {
			return;
		}
		logger.info("【初始化数据源】");
		T datasource = this.createDataSource(configMap.get(RoutingDataSourceContext.DATASOURCE_DRIVER).toString(),
				configMap.get(RoutingDataSourceContext.DATASOURCE_URL).toString(),
				configMap.get(RoutingDataSourceContext.DATASOURCE_USERNAME).toString(),
				configMap.get(RoutingDataSourceContext.DATASOURCE_PASSWORD).toString());
		this.addTargetDataSource(configMap.get(RoutingDataSourceContext.DATASOURCE_KEY).toString(), datasource);
	}
